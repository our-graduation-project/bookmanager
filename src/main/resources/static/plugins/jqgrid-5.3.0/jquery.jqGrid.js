...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		this._getOrder=function(data,by,dir,type, dfmt, sfunc){
			...
			if(type === undefined ) { type = "static.mydist.text"; }
		...
		this._compareValues=function(func,f,v,how,t){
			...
			//var val=v===null?f:v,
			var val =v,
			swst = t.stype === undefined ? "static.mydist.text" : t.stype;
			if(v !== null) {
			switch(swst) {
				case 'int':
				case 'integer':
		...
		this.orderBy=function(by,dir,stype, dfmt, sfunc){
			...
			if(stype == null) { stype = "static.mydist.text"; }
	...
	return this.each( function() {
		...
		addLocalData = function( retAll ) {
			...
			$.each(ts.p.colModel,function(){
				sorttype = this.sorttype || "static.mydist.text";
			...
			if(ts.p.treeGrid && ts.p._sort) {
				$(ts).jqGrid("SortTree", st, ts.p.sortorder, cmtypes[st].stype || 'static.mydist.text', cmtypes[st].srcfmt || '');
				return;
			}
			var compareFnMap = {
		...
		buildColItems = function (top, left, parent) {
			...
			if(!$.jgrid.isElementInViewport($("#col_menu")[0])){
				$("#col_menu").css("templates.common.left", - parseInt($("#column_menu").innerWidth(),10) +"px");
			}
			if($.fn.html5sortable()) {
		...
		buildSearchBox = function (index, top, left, parent) {
			...
			} else if(cm.sorttype === 'static.mydist.text') {
				so = stropts;
			} else {
				so = numopts;
			}
			...
			if(!$.jgrid.isElementInViewport($("#search_menu")[0])){
				$("#search_menu").css("templates.common.left", -parseInt($("#column_menu").innerWidth(),10) +"px");
			}
		...
		buildColMenu = function( index, left, top ){
			...
			if(ts.p.direction === "ltr") {
				...
				$("#column_menu").css("templates.common.left", (left- wcm)+'px');
			}
			...
			if( parseFloat($("#column_menu").css("templates.common.left")) < 0 ) {
				$("#column_menu").css("templates.common.left", $(ts).css("templates.common.left") );
			}
		},
		colTemplate;
		...
		.append("<span class='ui-jqgrid-headlink " + icoo +"'></span>").css((dir==="rtl"?"templates.common.left":"right"),"0px") : "";
...
///// end toolbar menu
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1] === "templates.common.top") {$(grid.uDiv).insertBefore(grid.hDiv);}
...
$.jgrid.extend({
	...
	setSortIcon : function(colname, position) {
		return this.each(function(){
			...
			if(pos>=0) {
				var thecol = $("tr.ui-jqgrid-labels th:eq("+pos+")",$t.grid.hDiv);
				if(position === 'templates.common.left') {
					thecol.find(".s-ico").css("float", "templates.common.left");
				} else {
	...
	editCell : function (iRow,iCol, ed, event){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
				...
				if (!cm.edittype) {cm.edittype = "static.mydist.text";}
				...
				if( $.inArray(cm.edittype, ['static.mydist.text','textarea','password','select']) > -1) {
	...
	hideModal : function (selector,o) {
		...
		if( o.formprop && thisgrid  && o.form) {
			...
			$(thisgrid).data(frmdata, {
				top:parseFloat($(selector).css("templates.common.top")),
				left : parseFloat($(selector).css("templates.common.left")),
				width : fw,
				height : fh,
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		if(rtlsup) {
			...
			$(".ui-jqdialog-titlebar-close",mh).css("templates.common.left",0.3+"em");
		} else {
			mw.dir = "ltr";
			$(".ui-jqdialog-title",mh).css("float","templates.common.left");
...
$.fn.jqFilter = function( arg ) {
	...
	var p = $.extend(true,{
		...
		numopts : ['eq','ne', 'lt', 'le', 'gt', 'ge', 'nu', 'nn', 'in', 'ni'],
		stropts : ['eq', 'ne', 'bw', 'bn', 'ew', 'en', 'cn', 'nc', 'nu', 'nn', 'in', 'ni'],
		strarr : ['static.mydist.text', 'string', 'blob'],
	...
	return this.each( function() {
		...
		for(i=0; i < len; i++) {
			...
			if( cl.stype ) {
				...
				cl.inputtype = 'static.mydist.text';
			}
			if( cl.sorttype ) {
				// grid compatibility
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.on('change',function() {
				...
				if(isIE && cm.inputtype === "static.mydist.text") {
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.stype === undefined) {this.stype='static.mydist.text';}
					...
					for (i = 0; i < l; i++) {
						...
						if( ia !== -1) {
							...
							if ( searchitem.length > 0) {
								...
								} else if (cm[ia].stype === "static.mydist.text") {
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				align = "templates.common.left", butleft ="";
				if($t.p.direction === "rtl") {
					align = "right";
...
$.jgrid.extend({
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if(!this.edittype) {
							this.edittype = "static.mydist.text";
						}
						if($t.p.autoencode) {
						...
						if( $.inArray(this.edittype, 
							['static.mydist.text','textarea','password','select', 
			...
			if(rtlb) {
				...
				$(".EditButton",frmtb+"_2").css("text-align","templates.common.left");
			}
			...
			p.saveicon = $.extend([true,"templates.common.left", styles.icon_save ],p.saveicon);
			p.closeicon = $.extend([true,"templates.common.left", styles.icon_close ],p.closeicon);
			...
			$.jgrid.viewModal("#"+$.jgrid.jqID(IDs.themodal),{
				...
				onHide :  function(h) {
					...
					$($t).data("formProp", {
						top:parseFloat($(h.w).css("templates.common.top")),
						left : parseFloat($(h.w).css("templates.common.left")),
						width : fw,
						height : fh,
						dataheight : $(frmgr).height(),
						datawidth: $(frmgr).width()
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				p.delicon = $.extend([true,"templates.common.left", styles.icon_del ],rp_ge[$t.p.id].delicon);
				p.cancelicon = $.extend([true,"templates.common.left", styles.icon_cancel ],rp_ge[$t.p.id].cancelicon);
				if(p.delicon[0]===true) {
	...
	navGrid : function (elem, p, pEdit, pAdd, pDel, pSearch, pView) {
		...
		p = $.extend({
			...
			position : "templates.common.left",
			closeOnEscape : true,
			beforeRefresh : null,
			afterRefresh : null,
			cloneToTop : false,
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("div.ui-jqgrid-sortable").each(function () {
					...
					if ($parent.is(":visible") && $parent.is(":has(span.ui-jqgrid-resize)")) {
						...
						$ts.css('templates.common.top', ($parent.height() - $ts.outerHeight()) / 2  - 4 +  'px');
					}
				});
			}
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						...
						if(cm[i].editable===true) {
							...
							if(!cm[i].edittype) { cm[i].edittype = "static.mydist.text"; }
							if(tmp === "&nbsp;" || tmp === "&#160;" || (tmp.length===1 && tmp.charCodeAt(0)===160) ) {tmp='';}
							var elc = $.jgrid.createEl.call($t,cm[i].edittype,opt,tmp,true,$.extend({},$.jgrid.ajaxOptions,$t.p.ajaxSelectOptions || {}));
							$(elc).addClass("editable inline-edit-cell");
							if( $.inArray(cm[i].edittype, ['static.mydist.text','textarea','password','select']) > -1) {
								$(elc).addClass( inpclass );
							}
		...
		return this.each(function(){
			...
			function pivot( data) {
				...
				for(i=0; i< pivotGrid.groupOptions.groupingView.groupField.length; i++) {
					...
					st = pivotOpt.xDimension[i].sorttype ? pivotOpt.xDimension[i].sorttype : 'static.mydist.text';
				...
				if(gridOpt.sortname) { // should be a part of xDimension
					...
					if(pivotGrid.groupOptions.sortname && len) {
						...
						st = pivotOpt.xDimension[len-1].sorttype ? pivotOpt.xDimension[len-1].sorttype : 'static.mydist.text';
...
setSubGrid : function () {
	return this.each(function (){
		...
		if(cm[0]) {
			...
			for(i=0;i<cm[0].name.length;i++) { cm[0].align[i] = cm[0].align[i] || 'templates.common.left';}
		}
	});
},
...
i=function(e,h,k,aR){
	return e.each(function(){
		...
		h.on('mousedown',{e:e,k:k},function(v){
			...
			M={
				X:p.left||f('templates.common.left')||0,
				Y:p.top||f('templates.common.top')||0,
			...
			if(E1 && d.k != 'd'){
				M1={
					X:p.left||f1('templates.common.left')||0,
					Y:p.top||f1('templates.common.top')||0,
...
//============================================================================
			...
			for ( j=0, ien=cm.length ; j<ien ; j++ ) {
				...
				align[cm[j].name] = cm[j].align || 'templates.common.left';
				i++;
			}
			var gh;
			if(o.includeGroupHeader && $t.p.groupHeader && $t.p.groupHeader.length) {
	...
	exportToHtml : function ( o ) {
		...
		this.each(function() {
			...
			for ( j=0, ien=cm.length ; j<ien ; j++ ) {
				...
				data.map[i] = j;
				data.align[i] = cm[j].align || 'templates.common.left';
				i++;
			}
